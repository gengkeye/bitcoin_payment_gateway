class CreateSuggestionEmailResultWorker
  ...
  def perform(email_id, interval)
  	Rails.logger.debug "email_id: #{email_id}, interval: #{interval}"
  	@email = SuggestionEmail.find_by(id: email_id)
  	last_price = ExchangeRate.get_last_price
  	Rails.logger.debug "email is present: #{ @email.present? }"
 	if @email.present?
 		@result = @email.results.build(time_interval: interval, fbase: @email.fbase, before_price: @email.last_price, now_price: last_price, gap: last_price - @email.last_price)
